...
export async function extractActiveTabArticle(): Promise<ExtractedArticle> {
  ...
  if (tab.url && !/^https?:/.test(tab.url)) {
    throw new ExtractionError('This page type can’t be summarized. Open an article or blog post.');
  }

  const request: ExtractArticleRequest = { type: 'EXTRACT_ARTICLE' };
  let response: ExtractArticleResponse;
  try {
    response = await chrome.tabs.sendMessage<ExtractArticleRequest, ExtractArticleResponse>(
      tab.id,
      request,
    );
  } catch {
